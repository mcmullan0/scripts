...
if ($opts{i})
{
    ...
    {
        my %header_data;
        my @headers_in_order = ("MSNP", "MSNP_Ind", "mSNP", "mSNP_Ind", "HSNP", "HSNP_Ind", "indel", "ambig", "total");
        ...
        foreach my $line (<INFILE>)
        {
            ...
            if ($line =~ m/^>/)
            {
                print "# Population level polyM data for:\n#$line\nMSNP,MSNP_Ind,mSNP,mSNP_Ind,HSNP,HSNP_Ind,Indel,Ambig,Total,\n";
            }
            else
            {
                my @bp = split(/,/, $line);
...
#                            print "\nThis should be the number of other bases = $temp2baseNo\n";
                ...
                # Print %header_data
                print "$header_data{MSNP},$header_data{MSNP_Ind},$header_data{mSNP},$header_data{mSNP_Ind},$header_data{'HSNP'},$header_data{'HSNP_Ind'},$header_data{'indel'},$header_data{'ambig'},$header_data{total}\n";     # if exists $polyM{$indel};
