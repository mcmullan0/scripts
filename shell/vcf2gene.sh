...
while getopts "r:g:nv:L:p:kq:d:o:R" opt
do
  case $opt in
    r)
      INFILE=$OPTARG
...
then
  ...
  echo -ne "#\t\t -n if reference.gff has already been generated by this script (consecutive num)\n"
...
## This was only neccassary for picard which I no longer use
FA='fa'
...
#    source jre-7.11
#    java -jar /tgac/software/testing/gatk/3.5.0/x86_64/bin/GenomeAnalysisTK.jar -T FastaAlternateReferenceMaker -R $FAS -V $IVCF -o $IVCF.fasta --use-iupac-sample
#    rm $IVCF"
#  done
########## Using VCFtools #####
  for IVCF in *-${PREFIX}.no-refref.no-missing.recode.vcf
  do
    submit-slurm.pl -j ${RANDSLURM}vcf-consensus-$IVCF -q ei-medium -t 0-02:00 -m 16G -i "source vcftools-0.1.13
    bgzip $IVCF
    tabix -p vcf ${IVCF}.gz
    cat $FAS | vcf-consensus -i ${IVCF}.gz > $IVCF.fasta
    rm ${IVCF}.gz ${IVCF}.gz.tbi"
  done

  echo "Awating production of individual reference fastas"
  HOLD=$(squeue -u mcmullam | grep -c "MM-${RANDSLURM}vc")
  COUNTER=0
  while (( "$HOLD" != "0" ))
  do
    sleep ${DURATION}m
  ...
  do
   submit-slurm.pl -j ${RANDSLURM}fas-$IFAS -m 4G -i "fastline.pl -i ${IFAS}-${PREFIX}.no-refref.no-missing.recode.vcf.fasta -a > ${IFAS}.filt.fas
    rm ${IFAS}-${PREFIX}.no-refref.no-missing.recode.vcf.fasta"
  done < $INDLIST
...
#  else
#    echo "Your gff has already been converted to consecutive numbers and is called ${GFFPREFIX}.contig-no.${GFFEXTENSION}?"
#    ls ${GFFPREFIX}.contig-no.${GFFEXTENSION}
#  fi
#  
...
#  do
#    submit-slurm.pl -j ${RANDSLURM}t2n-$FASNO -m 4G -i "awk '{print \$1}' $FASNO > test-$FASNO
#    mv test-$FASNO $FASNO"
#  done
#  echo "Awaiting production of contig name to number fastas"
#  HOLD=$(squeue -u mcmullam | grep -c "MM-${RANDSLURM}t2")
#  COUNTER=0
#  while [[ "$HOLD" != "0" ]]
#  do
#    sleep ${DURATION}m
#    HOLD=$(squeue -u mcmullam | grep -c "MM-${RANDSLURM}t2")
#    COUNTER=$(( $COUNTER + $DURATION ))
#    echo -e "... production of contig number fastas\n... ... $HOLD jobs remaining ($COUNTER minutes passed)"
#  done
#  echo -e "Production of contig number fastas complete"
  ...
  while read INF
  do
    submit-slurm.pl -j ${RANDSLURM}genes-$INF -m 4G -i "source cufflinks-2.2.1_gk
#    gffread ${GFFPREFIX}.contig-no.${GFFEXTENSION} -g ${INF}.fas -x $INF.cds.fasta
    gffread ${GFFPREFIX}.${GFFEXTENSION} -g ${INF}.fas -x $INF.cds.fasta
    fastline.pl -i ${INF}.cds.fasta -a | awk '{print \$1}' > ${INF}.cds.fas
    rm ${INF}.cds.fasta"
  done < ind-fas.list
  echo "Awaiting production of gene sequences per individual"
  HOLD=$(squeue -u mcmullam | grep -c "MM-${RANDSLURM}ge")
  COUNTER=0
  while [[ "$HOLD" != "0" ]]
  do
    sleep ${DURATION}m
    HOLD=$(squeue -u mcmullam | grep -c "MM-${RANDSLURM}ge")
    COUNTER=$(( $COUNTER + $DURATION ))
    echo -e "... production of individulas gene fastas\n... ... $HOLD jobs remaining ($COUNTER minutes passed)"
  done
  echo -e "Production of individuals gene fastas complete"
