...
then 
  ...
  do
    submit-slurm.pl -j ${RANDSLURM}ref-maker-$IVCF -m 20480 -i "source gatk-3.5.0
    source jre-7.11
  ...
  while read IFAS
  do
   submit-slurm.pl -j ${RANDSLURM}fas-$IFAS -i "fastline.pl -i ${IFAS}-${PREFIX}.no-refref.no-missing.recode.vcf.fasta -a > ${IFAS}.filt.fas
